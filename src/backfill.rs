    ...
    for (mint, launchpad, pool) in rows {
        ...
        match launchpad.as_str() {
            ...
            "pumpfun" => {
                ...
                let usd: Option<f64> = sqlx::query_scalar("SELECT s.price_usd * s.multiplier FROM tokens t JOIN stocks s ON s.mint=t.quote_mint WHERE t.mint=$1").bind(&mint).fetch_optional(db).await?.flatten();
